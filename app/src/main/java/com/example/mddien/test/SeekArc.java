...
public class SeekArc extends View {
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        ...
        mThumb = res.getDrawable(R.drawable.selector_seek_arc_control);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawArc(mArcRect, arcStart, mProgressSweep, false,
                mProgressPaint);
        float degree = arcStart + mProgressSweep;
        mIndicatorPaint.setStyle(Paint.Style.STROKE);
        canvas.drawLine((float) (mTranslateX+Math.cos(Math.toRadians(degree))*(mArcRadius+30)),
                (float) (mTranslateY+Math.sin(Math.toRadians(degree))*(mArcRadius+30)),
                (float) (mTranslateX+Math.cos(Math.toRadians(degree))*mArcRadius),
                (float) (mTranslateY+Math.sin(Math.toRadians(degree))*mArcRadius),
                mIndicatorPaint);
        mIndicatorPaint.setStyle(Paint.Style.FILL);
        canvas.drawPath(getTrianglePath(), mIndicatorPaint);
        if (mEnabled) {
            // Draw the thumb nail
    ...
    private Path getTrianglePath() {
        float degree = mStartAngle + mAngleOffset + mRotation + mProgressSweep;
        float topPointX = (float) (mTranslateX+Math.cos(Math.toRadians(degree))*(mArcRadius - 40));
        float topPointY = (float) (mTranslateY+Math.sin(Math.toRadians(degree))*(mArcRadius - 40)) ;
        float hX = (float) (mTranslateX+Math.cos(Math.toRadians(degree))*(mArcRadius - 50));
        float hY = (float) (mTranslateY+Math.sin(Math.toRadians(degree))*(mArcRadius - 50)) ;
        // n(topPointX - hX, topPointY - hY) -> t1(hY-topPointY, topPointX - hX) t2(topPointY - hy, hX - topPointX)
        float leftX = hX + (hY - topPointY);
        float leftY = hY + topPointX - hX;
        float rightX = hX + topPointY - hY;
        float rightY = hY + hX - topPointX;
        Path path = new Path();
